...
for i in range(NUM_SOFT_BUNNIES):
  ...
  YOUNG_VALUE = 2000.0 + random.random() * 2000
  MU_LAME_VALUE = YOUNG_VALUE / (2 * (1 + POISSON_VALUE))
  LAMBDA_LAME_VALUE = YOUNG_VALUE * POISSON_VALUE / ((1 + POISSON_VALUE) * (1 - 2 * POISSON_VALUE))
...
for i in range(NUM_SOFT_BUNNIES):
  ...
  position_softs.append(np.dot(position_copy * (random.random() * 1.0), rotation_matrix) + translation)
...
for i in range(NUM_ABD_BUNNIES):
  ...
  if i != NUM_ABD_BUNNIES - 1:
    position_abds.append(position * (random.random() * 1.5))
  else:
    position_abds.append(position * 1.5)
...
cage_points = cage_points / 80.0
cage_points[:, 1] += prev_center[1] + 0.8
cage_points[:, 2] *= -1
cage_points[:, 0] *= -1
cage_points[:, 0] -= 0.2
cage_points[:, 2] += 0.3
...
snh_cage = stable_neo_hookean(ctrp, ctp, mu, lam, dt)
ct.addAttribute("snh_cage", computed_attribute = snh_cage)
...
s0.addMinimizeTarget([abds_abd_matrices, abds_translations, vertices_soft_position, cvp])
# s0.addMinimizeTarget([abds_abd_matrices, abds_translations, vertices_soft_position])
