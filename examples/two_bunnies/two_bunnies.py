...
KAPPA_VALUE = 1e-6 # for collision
##################################################
## read the bunny file
##################################################
...
velocities[:position.shape[0] //2, 0] = 3
velocities[position.shape[0] // 2:, 0] = -7
bunnies.vertices["velocity"].updateValue(velocities)

bunnies.vertices.addAttribute("mass", rows = 1, cols = 1)
bunnies.vertices["mass"].updateValue(np.ones((position.shape[0]), dtype=np.float64) * 5.0)
...
mu.updateValue([200.0])
lam.updateValue([100.0])
...
for i in range(2000):
  ...
  while True:
    ...
    energies_before = sum(snh_energy.compute().value.get()) + \
                      sum(inertia_energy.compute().value.get()) + \
                      sum(pp_energy.compute().value.get()) + \
                      sum(pe_energy.compute().value.get()) + \
                      sum(pt_energy.compute().value.get()) + \
                      sum(ee_energy.compute().value.get())
    print("energy before is", energies_before)

    # we perform CCD here
    d_position = result[0]
    max_dposition = abs_max_reduce(d_position).get()
    print("max dposition is", max_dposition)
    step_taken = 1.0
    ...
    if step_taken == 1.0:
      ...
      while substep <= 32:
        bunnies.vertices["position"].updateValue(position_copy - d_position * step_taken, deepCopy = True)
        ccd.cd(bunnies.vertices["position"].value, DHAT_VALUE) # perform collision detection
        print("the merged counts are:", ccd.cp_num.get().tolist())
        pp_count, pe_count, pt_count, ee_count = ccd.separated_counts
    ...
    # bunnies.vertices["position"].updateValue(bunnies.vertices["position"].value - d_position * step_taken, deepCopy = True)
  ...
  bunny_poly.save(f"outputs/bunny_{i:04d}.obj")
  plotter.render()
  plotter.update()
