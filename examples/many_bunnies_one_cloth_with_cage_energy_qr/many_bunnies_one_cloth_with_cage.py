...
from helpers import extract_surface_triangles, inertia, extract_edges_from_triangles, print_mem, precompute_cage_qr, stable_neo_hookean_cage_qr, save_video
import math
import numpy as np
import sys
import os
...
# Resolve shared mesh/CCD assets from this script, and keep caches/outputs local.
...
SAVE_FRAMES = True
SAVE_VIDEO = True
VIDEO_FPS = 100  # Real-time playback for dt = 0.01; use 30 for a slower inspection video.
KEEP_FINAL_WINDOW_OPEN = True
MAX_CD_PAIRS = 2_000_000
...
triangle_indices_cloth = mapping[triangle_indices_cloth]
...
##################################################################
cage_npz = np.load(EXAMPLE_DIR / "weights.npz")
...
bunny_faces = []
bunny_vertices = []
f = open(DATA_DIR / "bunny_small.obj", 'r')
for line in f:
...
##################################################################
s0 = scene("many_bunnies_one_cloth_cage_qr")
...
# Precompute the rest-space QR factors at all eight Gauss points per hex.
# B = dN/dX = Q R, so F = x^T B = (x^T Q) R.
quadrature_vertices, qr_basis_values, qr_transform_values, quadrature_volumes = precompute_cage_qr(cage_points, cages)
NUM_QUADRATURE_POINTS = quadrature_vertices.shape[0]
quadrature = cage.addPrimitive("quadrature_points", numInstances = NUM_QUADRATURE_POINTS)
quadrature2cv = quadrature.addConnectivity("quadrature2cv", cv, quadrature_vertices, 8)
quadrature_positions = quadrature.addAttribute("positions", through = quadrature2cv, source = cvp)
qr_basis = quadrature.addConstant("qr_basis", rows = 8, cols = 3)
qr_basis.updateValue(qr_basis_values.reshape(-1))
reduced_position = quadrature.addAttribute("reduced_position", computed_attribute = quadrature_positions.transpose() * qr_basis)

# This one-to-one JOIN makes the nine reduced coordinates the energy's inner
# variables. Only cvp is optimized; reduced_position stays a computed attribute.
quadrature_energy = cage.addPrimitive("quadrature_energy", numInstances = NUM_QUADRATURE_POINTS)
energy2quadrature = quadrature_energy.addConnectivity("energy2quadrature", quadrature, np.arange(NUM_QUADRATURE_POINTS, dtype=np.uint32).reshape(-1, 1), 1)
energy_reduced_position = quadrature_energy.addAttribute("reduced_position", through = energy2quadrature, source = reduced_position).resize(3, 3)
qr_transform = quadrature_energy.addConstant("qr_transform", rows = 3, cols = 3)
qr_transform.updateValue(qr_transform_values.reshape(-1))
quadrature_volume = quadrature_energy.addConstant("quadrature_volume", rows = 1, cols = 1)
quadrature_volume.updateValue(quadrature_volumes)
...
# Same Hex8 stable neo-Hookean energy, expressed in nine QR coordinates.
snh_cage = stable_neo_hookean_cage_qr(energy_reduced_position, qr_transform, quadrature_volume, mu, lam, dt)
quadrature_energy.addAttribute("stable_neo_hookean_cage_qr", computed_attribute = snh_cage)
...
NUM_COLLISION_SURFACE_VERTICES = NUM_BUNNY_SURFACE_INDICES * (NUM_ABD_BUNNIES + NUM_SOFT_BUNNIES) + positions_cloth.shape[0] + bunny_vertices.shape[0]
NUM_COLLISION_VERTICES = NUM_BUNNY_VERTICES * (NUM_ABD_BUNNIES + NUM_SOFT_BUNNIES) + positions_cloth.shape[0] + bunny_vertices.shape[0]
ccd = CCD(NUM_COLLISION_SURFACE_VERTICES, NUM_COLLISION_VERTICES, max_ccd_pairs = MAX_CCD_PAIRS, max_cd_pairs = MAX_CD_PAIRS, mesh_indices = mesh_indices)
...
## plot the bunnies
##################################################################
plotter = pv.Plotter(window_size=[1920, 1080], title="Many bunnies and one cloth - QR cage energy (9 x 9)")
all_vertices_computed = collision_vertices_position.compute().value.get().reshape(-1, 3)
ABD_END = NUM_ABD_BUNNIES * NUM_BUNNY_VERTICES
SOFT_END = (NUM_ABD_BUNNIES + NUM_SOFT_BUNNIES) * NUM_BUNNY_VERTICES
CLOTH_END = SOFT_END + positions_cloth.shape[0]
abd_triangles = triangle_indices_all[:NUM_ABD_BUNNIES * NUM_BUNNY_SURFACE_TRIANGLES]
soft_triangles = triangle_indices_all[NUM_ABD_BUNNIES * NUM_BUNNY_SURFACE_TRIANGLES:(NUM_ABD_BUNNIES + NUM_SOFT_BUNNIES) * NUM_BUNNY_SURFACE_TRIANGLES] - ABD_END
abd_poly = pv.PolyData(all_vertices_computed[:ABD_END], np.column_stack([np.full(abd_triangles.shape[0], 3), abd_triangles]))
soft_poly = pv.PolyData(all_vertices_computed[ABD_END:SOFT_END], np.column_stack([np.full(soft_triangles.shape[0], 3), soft_triangles]))
cloth_poly = pv.PolyData(all_vertices_computed[SOFT_END:CLOTH_END], np.column_stack([np.full(triangle_indices_cloth.shape[0], 3), triangle_indices_cloth]))
bunny_poly = pv.PolyData(all_vertices_computed[CLOTH_END:], np.column_stack([np.full(bunny_faces.shape[0], 3), bunny_faces]))
cage_edges = np.unique(np.sort(cage_edges, axis=1), axis=0)
cage_poly = pv.PolyData(cage_points.copy(), lines=np.column_stack([np.full(cage_edges.shape[0], 2), cage_edges]))
plotter.add_mesh(abd_poly, color="lightblue", label="Affine bunny")
plotter.add_mesh(soft_poly, color="lightgreen", label="Soft bunny")
plotter.add_mesh(cloth_poly, color="pink", opacity=0.5, label="Cloth")
plotter.add_mesh(bunny_poly, color="royalblue", opacity=0.85, label="Caged bunny")
plotter.add_mesh(cage_poly, color="tomato", line_width=2, opacity=0.3, label="Cage")
plotter.add_legend()
plotter.add_axes()
plotter.set_background("white")
# Fixed 45-degree azimuth, with a low elevation to show cloth contact/deformation.
# Include space below the initial cloth so the camera does not track the motion.
view_min = all_vertices_computed.min(axis=0)
view_max = all_vertices_computed.max(axis=0)
view_min[1] -= 0.75 * (view_max[1] - view_min[1])
view_center = 0.5 * (view_min + view_max)
view_radius = np.linalg.norm(view_max - view_min)
plotter.camera_position = [view_center + view_radius * np.array([1.0, 0.4, 1.0]), view_center, (0, 1, 0)]
plotter.reset_camera(bounds=(view_min[0], view_max[0], view_min[1], view_max[1], view_min[2], view_max[2]))
plotter.add_text("Initial configuration | cage inner Hessian: 9 x 9", name="status", font_size=14, color="black")
...
for i in range(NUM_FRAMES):
  ...
  # Refresh every surface and the control cage from the converged frame.
  all_vertices_computed = collision_vertices_position.compute().value.get().reshape(-1, 3)
  abd_poly.points = all_vertices_computed[:ABD_END]
  soft_poly.points = all_vertices_computed[ABD_END:SOFT_END]
  cloth_poly.points = all_vertices_computed[SOFT_END:CLOTH_END]
  bunny_poly.points = all_vertices_computed[CLOTH_END:]
  cage_poly.points = cvp.value.get().reshape(-1, 3)
  plotter.add_text(f"Frame {i + 1}/{NUM_FRAMES} | t={(i + 1) * DT_VALUE:.2f} s | cage inner Hessian: 9 x 9", name="status", font_size=14, color="black")
  plotter.update()
  if SAVE_FRAMES or SAVE_VIDEO:
    plotter.screenshot(str(OUTPUT_DIR / f"frame_{i:04d}.jpg"))
  completed_frames += 1
end = time.time()
print("Total time: ", end - start)

# Encode exactly the frames from this run, even if older JPGs remain in outputs.
if SAVE_VIDEO and completed_frames > 0:
  save_video(OUTPUT_DIR, completed_frames, VIDEO_FPS)

# Keep the final configuration visible until the window is closed.
if not plotter._closed:
  if KEEP_FINAL_WINDOW_OPEN:
