"""Local copy of the existing lagged IPC friction energies; all derivatives use YASPS."""
...
# Smooth the tangential displacement and evaluate the shared friction potential.
def smooth_friction_phi(y2, fDhat, dt):
  ...
  return attribute.select(y2 > fricDHat, nonsmooth, smooth)


def friction_energy_from_rel_dx(relDX, fDhat, dt, frictionRate, tangent0, tangent1, lambda_last_h):
...
# Point-triangle coordinates, tangent basis, force magnitude, and energy.
def closest_point_coord_and_tangent_basis_pt(position):
  ...
  B = attribute.to_array([e01[0], e02[0], e01[1], e02[1], e01[2], e02[2]], rows=3, cols=2)
  ...
  coord = attribute.to_array([beta1, beta2], rows=1, cols=2)

  tangent_basis = attribute.to_array([tangent0[0], tangent0[1], tangent0[2], tangent1[0], tangent1[1], tangent1[2]], rows=2, cols=3)
...
def friction_energy_pt(position, old_position, fDhat, dt, frictionRate, coord, tangent0, tangent1, lambda_last_h):
  relDX = rel_dx_pt(position, old_position, coord)

  return friction_energy_from_rel_dx(relDX, fDhat, dt, frictionRate, tangent0, tangent1, lambda_last_h)

# Point-edge coordinates, tangent basis, force magnitude, and energy.
def closest_point_coord_and_tangent_basis_pe(position):
  ...
  coord = attribute.to_array([eta, scalar_zero()], rows=1, cols=2)
...
def friction_energy_pe(position, old_position, fDhat, dt, frictionRate, coord, tangent0, tangent1, lambda_last_h):
  relDX = rel_dx_pe(position, old_position, coord)

  return friction_energy_from_rel_dx(relDX, fDhat, dt, frictionRate, tangent0, tangent1, lambda_last_h)


# Edge-edge coordinates, tangent basis, force magnitude, and energy.
def closest_point_coord_and_tangent_basis_ee(position):
  ...
  A = attribute.to_array([e01.dot(e01), -e23.dot(e01), -e23.dot(e01), e23.dot(e23)], rows=2, cols=2)

  rhs = attribute.to_array([-e20.dot(e01), e20.dot(e23)], rows=2, cols=1)
  ...
  coord = attribute.to_array([gamma1, gamma2], rows=1, cols=2)
...
def friction_energy_ee(position, old_position, fDhat, dt, frictionRate, coord, tangent0, tangent1, lambda_last_h):
  relDX = rel_dx_ee(position, old_position, coord)

  return friction_energy_from_rel_dx(relDX, fDhat, dt, frictionRate, tangent0, tangent1, lambda_last_h)

# Point-point coordinates, tangent basis, force magnitude, and energy.
def closest_point_coord_and_tangent_basis_pp(position):
  ...
  x_cross = attribute.to_array([0.0, -v01[2], v01[1]], rows=1, cols=3)

  y_cross = attribute.to_array([v01[2], 0.0, -v01[0]], rows=1, cols=3)
  ...
  tangent0_raw = attribute.to_array([tangent0_raw_x, tangent0_raw_y, tangent0_raw_z], rows=1, cols=3)

  tangent0 = normalize(tangent0_raw)
  tangent1 = normalize(v01.cross(tangent0_raw))

  coord = attribute.to_array([scalar_zero(), scalar_zero()], rows=1, cols=2)
...
def friction_energy_pp(position, old_position, fDhat, dt, frictionRate, coord, tangent0, tangent1, lambda_last_h):
