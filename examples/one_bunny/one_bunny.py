...
f.close()
position = np.array(position, dtype = np.float64) * BUNNY_SCALE_FACTOR
...
bunnies.addPrimitive("fixed_vertices", numInstances = 1)
bunnies.addPrimitive("moving_vertices", numInstances = position.shape[0] - 1)
bunnies.addPrimitiveUnion("vertices", [bunnies.fixed_vertices, bunnies.moving_vertices])
...
bunnies.moving_vertices.addConstant("rest_position", rows = 3, cols = 1)
bunnies.moving_vertices["rest_position"].updateValue(position[1:, :] / BUNNY_SCALE_FACTOR)
bunnies.moving_vertices.addAttribute("position", rows = 3, cols = 1)
bunnies.moving_vertices["position"].updateValue(position[1:, :])
bunnies.moving_vertices.addConstant("last_position", rows = 3, cols = 1)
bunnies.moving_vertices["last_position"].updateValue(position[1:, :])
bunnies.moving_vertices.addConstant("velocity", rows = 3, cols = 1)
velocities = np.zeros_like(position[1:, :], dtype=np.float64)
bunnies.moving_vertices["velocity"].updateValue(velocities)
bunnies.moving_vertices.addConstant("mass", rows = 1, cols = 1)
bunnies.moving_vertices["mass"].updateValue(np.ones((position.shape[0] - 1), dtype=np.float64) * 0.05)

# add attributes for fixed vertices
bunnies.fixed_vertices.addAttribute("position", rows = 3, cols = 1)
bunnies.fixed_vertices["position"].updateValue(position[:1, :])
bunnies.fixed_vertices.addConstant("rest_position", rows = 3, cols = 1)
bunnies.fixed_vertices["rest_position"].updateValue(position[:1, :] / BUNNY_SCALE_FACTOR)
bunnies.fixed_vertices.addConstant("last_position", rows = 3, cols = 1)
bunnies.fixed_vertices["last_position"].updateValue(position[:1, :])
bunnies.fixed_vertices.addConstant("velocity", rows = 3, cols = 1)
bunnies.fixed_vertices["velocity"].updateValue(np.zeros((1, 3), dtype=np.float64))
...
ccd.init_edges(position_gpu, position_gpu / BUNNY_SCALE_FACTOR, edge_indices_gpu, edge_indices.shape[0])
...
inertia = inertia(bunnies.moving_vertices["last_position"], bunnies.moving_vertices["velocity"], dt, bunnies.moving_vertices["position"], bunnies.moving_vertices["mass"])
inertia_energy = bunnies.moving_vertices.addAttribute("inertia", computed_attribute = inertia)
...
s0.addMinimizeTarget([bunnies.moving_vertices["position"]])
...
bunny_poly1 = pv.PolyData(position, cells)
plotter = pv.Plotter()
plotter.add_mesh(bunny_poly1, color='blue', opacity = 0.5)
...
for i in range(2000):
  ...
  while True:
    ...
    position_copy[3:].set(bunnies.moving_vertices["position"].value)
    direction_copy[3:].set(d_position)
    print("max direction is", abs_max_reduce(direction_copy).get())
    ...
    # largest_step = 1.0
    # if (largest_step != 1):
    #   print("largest step size is", largest_step)
    #   exit()
    # largest_step = 1.0
    ...
    while substep <= 8:
      ...
      bunnies.moving_vertices["position"].updateValue(computed_position[3:], deepCopy = True)
    ...
    # new_positions = bunnies.vertices["position"].compute().value.get().reshape(-1, 3)
    # bunny_poly1.points = new_positions
    # plotter.render()
    # plotter.update()
    ...
    if max_grad < 1e-1:
      ...
      break
    inner_iteration += 1
  new_velocities = (bunnies.moving_vertices["position"].value - bunnies.moving_vertices["last_position"].value) / DT_VALUE
  bunnies.moving_vertices["velocity"].updateValue(new_velocities)
  new_positions = bunnies.vertices["position"].compute().value.get().reshape(-1, 3)
  bunny_poly1.points = new_positions
