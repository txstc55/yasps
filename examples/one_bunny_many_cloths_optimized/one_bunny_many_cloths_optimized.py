...
bdg = bunnies_soft.addPrimitive("deformation_gradient", numInstances = NUM_SOFT_BUNNIES * NUM_BUNNY_TETS)
bdg2bt = bdg.addConnectivity("bdg2bt", tets_softs, np.arange(NUM_SOFT_BUNNIES * NUM_BUNNY_TETS), 1)
...
bdg.addConstant("mu_softs", rows = 1, cols = 1)
bdg.addConstant("lambda_softs", rows = 1, cols = 1)
bdg["mu_softs"].updateValue(np.array([[MU_VALUE_SOFTS[i]] * NUM_BUNNY_TETS for i in range(NUM_SOFT_BUNNIES)], dtype = np.float64).flatten())
bdg["lambda_softs"].updateValue(np.array([[LAMBDA_VALUE_SOFTS[i]] * NUM_BUNNY_TETS for i in range(NUM_SOFT_BUNNIES)], dtype = np.float64).flatten())
...
row0 = triangles_cloth_positions.row(0)
row1 = triangles_cloth_positions.row(1)
row2 = triangles_cloth_positions.row(2)
x0 = row1 - row0
x1 = row2 - row0
F = attribute.to_array([x0[0], x0[1], x0[2], x1[0], x1[1], x1[2]], rows = 2, cols = 3)
btf = cloth.addAttribute("F", computed_attribute = F)

cdg = cloth.addPrimitive("deformation_gradient", numInstances = triangle_indices_cloth.shape[0])
cdg2ct = cdg.addConnectivity("cdg2ct", triangles_cloth, np.arange(triangle_indices_cloth.shape[0]), 1)
cdg_F = cdg.addAttribute("F", through = cdg2ct, source = btf)
cdg_F = cdg_F.resize(2, 3)

cdgrp = cdg.addAttribute("rest_positions", through = cdg2ct, source = triangles_cloth_rest_positions)
...
e_init = edges_cloth_rest_positions.row(1) - edges_cloth_rest_positions.row(0)
a_init = edges_cloth_rest_positions.row(2) - edges_cloth_rest_positions.row(0)
b_init = edges_cloth_rest_positions.row(3) - edges_cloth_rest_positions.row(0)
g_init = attribute.to_array([e_init[0], e_init[1], e_init[2], a_init[0], a_init[1], a_init[2], b_init[0], b_init[1], b_init[2]], rows = 3, cols = 3)
edges_cloths_g_init = edges_cloth.addAttribute("g_init", computed_attribute = g_init)

e = edges_cloth_positions.row(1) - edges_cloth_positions.row(0)
a = edges_cloth_positions.row(2) - edges_cloth_positions.row(0)
b = edges_cloth_positions.row(3) - edges_cloth_positions.row(0)
g = attribute.to_array([e[0], e[1], e[2], a[0], a[1], a[2], b[0], b[1], b[2]], rows = 3, cols = 3)
edges_cloths_g = edges_cloth.addAttribute("g", computed_attribute = g)

bedg = cloth.addPrimitive("edge_deformation_gradient", numInstances = edge_to_vertices_cloth.shape[0])
bedg2ec = bedg.addConnectivity("bedg2ec", edges_cloth, np.arange(edge_to_vertices_cloth.shape[0]), 1)
bedg_g = bedg.addAttribute("g", through = bedg2ec, source = g)
bedg_g = bedg_g.resize(3, 3)

bedg_g_init = bedg.addAttribute("g_init", through = bedg2ec, source = g_init)
bedg_g_init = bedg_g_init.resize(3, 3)
...
snh_softs = stable_neo_hookean_modified(bdg_F, bdg_TB, bdg["mu_softs"], bdg["lambda_softs"], dt)
...
from helpers import bending_modified
bending_energy = bending_modified(bedg_g, bedg_g_init, bending_stiffness, dt)
edges_cloth.addAttribute("bending_energy_modified", computed_attribute = bending_energy)

from helpers import baraff_witkin_modified
baraff_witkin_energy = baraff_witkin_modified(cdgrp, cdg_F, stretch, shear, thickness, dt)
triangles_cloth.addAttribute("baraff_witkin_energy_modified", computed_attribute = baraff_witkin_energy)
...
cloth_triangles_base = cloth_triangles[:int(cloth_triangles.shape[0] / NUM_CLOTH), ]
cloth_triangles_top = cloth_triangles[int(cloth_triangles.shape[0] / NUM_CLOTH):, ]


cells_soft = np.hstack([np.full((soft_triangles.shape[0], 1), 3), soft_triangles])
cells_cloth_base = np.hstack([np.full((cloth_triangles_base.shape[0], 1), 3), cloth_triangles_base])
cells_cloth_top = np.hstack([np.full((cloth_triangles_top.shape[0], 1), 3), cloth_triangles_top])
...
cloth_base_poly = pv.PolyData(cloth_vertices_computed, cells_cloth_base)
cloth_top_poly = pv.PolyData(cloth_vertices_computed, cells_cloth_top)

plotter.add_mesh(soft_poly, color = "lightgreen")
plotter.add_mesh(cloth_base_poly, color = "pink", opacity = 0.5)
plotter.add_mesh(cloth_top_poly, color = "lightblue", opacity = 0.5)
...
for i in range(200):
  ...
  #     exit(1)
    ...
    cloth_base_poly.points = cloth_vertices_computed
    cloth_top_poly.points = cloth_vertices_computed
    plotter.render()
    plotter.update()
  ...
  soft_poly.save(f"meshes/bunny_{i:04d}.obj")
  cloth_base_poly.save(f"meshes/cloth_base_{i:04d}.obj")
  cloth_top_poly.save(f"meshes/cloth_top_{i:04d}.obj")
  # # save the mesh obj file
