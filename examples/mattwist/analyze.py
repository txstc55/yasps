...
def extract_time(file):
  ...
  print("Last v count is:", last_v_count)
  print("Last min memory is:", last_min_memory)
  min_memory_per_10k = 0 if last_v_count == 0 else ((min_memory - last_min_memory) / (num_v - last_v_count)) * 10000
  increased_memory_per_cp = ((max_memory - min_memory) / maximum_cd_pairs) * 100000
  last_max_memory = max_memory
  last_min_memory = min_memory
  last_v_count = num_v
  print("min memory per 10k vertices is:", min_memory_per_10k)
  print("increased memory per collision pair is:", increased_memory_per_cp)
  ...
  result = {
    ...
    "min_memory_per_10k": min_memory_per_10k,
    "increased_memory_per_cp": increased_memory_per_cp
  }
  return result
...
def latex_row(stats):
  return (
    f"{fmt_int(stats['num_v'])} & "
    f"{fmt_int(stats['num_f'])} & "
    f"{fmt_int(stats['num_e'])} & "
    f"{fmt_int(stats['maximum_cd_pairs'])} & "
    f"{fmt_float(stats['ccd_cd_total'], 2)} & "
    f"{fmt_float(stats['diff_total'], 2)} & "
    f"{fmt_float(stats['cg_total'], 2)} & "
    f"{fmt_float(stats['ccd_memory'], 2)} & "
    f"{fmt_float(stats['yasps_memory_min'], 2)} & "
    f"{fmt_float(stats['yasps_memory_max'], 2)} & "
    # f"{fmt_float(stats['yasps_memory_average'], 2)} & "
    f"{fmt_float(stats['min_memory_per_10k'], 2)} & "
    f"{fmt_float(stats['increased_memory_per_cp'], 4)}\\\\ \\hline"
  )
...
def print_latex_table(stats_list):
  ...
  print(r"\begin{tabularx}{\textwidth}{|l|l|l|X|X|X|X|r|r|r|}")
  print(r"    \hline")
  print(r"    \thead{\# Vertices} & \thead{\# Faces} & \thead{\# Edges} & \thead{Max \#\\Collision Pairs} & \thead{CCD \& CD\\Total(s)} & \thead{Diff\\Total(s)} & \thead{CG\\Total(s)} & \thead{CCD\\Memory (MB)} & \thead{Min YASPS\\Memory (MB)} & \thead{Max YASPS\\Memory (MB)} \\ \hline \hline")
...
if __name__ == "__main__":
  log_files = [f"mat_twist_{i}.log" for i in range(1, 11)]
