...
def compute_rest_shape(rest_triangle_pos):
    """
    Compute the rest shape matrix M for each triangle.

    Args:
        rest_triangle_pos (torch.Tensor): Tensor of shape (T, 3, 3), where T is the number of triangles,
                                          3 vertices per triangle, and 3 coordinates per vertex.

    Returns:
        torch.Tensor: Rest shape matrix M of shape (T, 2, 2) for each triangle.
    """
    # Number of triangles
