...
def stable_neo_hookean_energy(current_tet_pos, rest_tet_pos, mu, lam):
  # Compute B from rest positions
  x0 = rest_tet_pos[:, 0, :]  # T x 3
  x1 = rest_tet_pos[:, 1, :]  # T x 3
  x2 = rest_tet_pos[:, 2, :]  # T x 3
  x3 = rest_tet_pos[:, 3, :]  # T x 3
  X0 = x1 - x0
  X1 = x2 - x0
  X2 = x3 - x0
  # B (T x 3 x 3)
  B = torch.stack([X0, X1, X2], dim=2) # T x 3 x 3
  # Compute volume
  detB = torch.linalg.det(B)
  vol = detB / 6.0
  # IB = (B^-1)
  IB = torch.linalg.inv(B)
  # Compute F from current positions similarly
  y0 = current_tet_pos[:, 0, :]  # T x 3
  ...
  Y1 = y2 - y0
  Y2 = y3 - y0
  F = torch.stack([Y0, Y1, Y2], dim=2) # T x 3 x 3
  # FI = F * IB
  FI = torch.bmm(F, IB)
  # J = det(FI)
  J = torch.linalg.det(FI)
  # IC = trace(FI^T FI) = sum(FI_ij^2)
  IC = torch.sum(FI * FI, dim=[1,2])  # T
  I3 = IC + 1.0
  # Compute the energy per tet
  shift = 1.0 + 0.75 * (mu / lam)
  energy_per_tet = vol * (0.5 * mu * (IC - 3.0) - 0.5 * mu * torch.log(I3)
                          + 0.5 * lam * (J - shift) * (J - shift))
  return energy_per_tet

mu = torch.tensor(2000.0, dtype=torch.float64, device=device, requires_grad=False)
lam = torch.tensor(1000.0, dtype=torch.float64, device=device, requires_grad=False)
...
def batch_tet_energy(current_tet_pos, rest_tet_pos, mu, lam, num_iterations=NUM_TETS):
  ...
  energies = stable_neo_hookean_energy(x, rest, mu, lam) * multipliers  # Shape: (num_iterations,)
...
# Warm-up GPU
for _ in range(10):
  _ = compiled_batch_tet_energy(current_single_tet, rest_single_tet, mu, lam)
_ = torch.autograd.grad(
  compiled_batch_tet_energy(current_single_tet, rest_single_tet, mu, lam),
  current_single_tet,
  create_graph=True
)
...
def energy_wrapper(x_flat):
  ...
  return compiled_batch_tet_energy(x_reshaped, rest_single_tet, mu, lam, num_iterations=NUM_TETS)
