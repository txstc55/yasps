...
HELPER_DIR = os.path.abspath(os.path.join(
  os.path.dirname(os.path.abspath(__file__)),
  "../bunny_dropping_on_ground",
))
...
FRICTION_HELPER_DIR = os.path.abspath(os.path.join(
  os.path.dirname(os.path.abspath(__file__)),
  "../dropping_in_container",
))
sys.path.insert(0, FRICTION_HELPER_DIR)
from friction_helpers import (
  closest_point_coord_and_tangent_basis_ee,
  closest_point_coord_and_tangent_basis_pe,
  closest_point_coord_and_tangent_basis_pp,
  closest_point_coord_and_tangent_basis_pt,
  friction_energy_ee,
  friction_energy_pe,
  friction_energy_pp,
  friction_energy_pt,
...
parser.add_argument(
  "--mass-adam-learning-rate",
  type=float,
  default=MASS_ADAM_LEARNING_RATE,
  help=(
    "Adam learning rate for the unbounded log-mass design. This is an "
    "optimizer scale, not a bound on the resulting mass."
  ),
)
parser.add_argument(
  "--poisson-step-size",
  dest="poisson_step_size",
  type=float,
  default=POISSON_STEP_SIZE,
  help=(
    "Maximum bounded-Poisson-logit update magnitude for any one element."
  ),
)
parser.add_argument(
  "--young-step-size",
  type=float,
  default=YOUNG_STEP_SIZE,
  help="Maximum bounded-Young-logit update magnitude for any one element.",
)
parser.add_argument(
  "--preview-only",
  action="store_true",
...
parser.add_argument(
  "--target-loss-final-frame-only",
  action="store_true",
  help=(
    "Apply the target-position loss only at the final converged frame; "
    "the motion loss remains active at every frame."
  ),
)
parser.add_argument(
  "--resume-checkpoint",
  ...
  default=None,
  help=(
    "Resume an interrupted Adam optimization from latest_checkpoint.npz. "
    "The checkpoint design is replayed once to reconstruct its converged "
    "state trajectory before the next backward pass."
  ),
)
...
if args.resume_checkpoint is not None and args.optimizer != "adam":
  raise ValueError("--resume-checkpoint currently supports only --optimizer adam.")
...
POISSON_BASE_LOGIT = np.log(
  POISSON_BASE_FRACTION / (1.0 - POISSON_BASE_FRACTION)
)
YOUNG_BASE_FRACTION = (
  (YOUNG_VALUE - YOUNG_MIN_VALUE)
  / (YOUNG_MAX_VALUE - YOUNG_MIN_VALUE)
)
YOUNG_BASE_LOGIT = np.log(
  YOUNG_BASE_FRACTION / (1.0 - YOUNG_BASE_FRACTION)
)
...
def mass_values_from_design(design_parameters, initial_vertex_masses):
  ...
  initial = np.broadcast_to(
    np.asarray(initial_vertex_masses, dtype=np.float64).reshape(-1),
    design.shape,
  )
...
def lump_tetrahedron_mass_to_vertices(
  positions,
  tetrahedra,
  total_mass,
):
  ...
  edge_matrices = np.stack([
    tetrahedron_positions[:, 1] - tetrahedron_positions[:, 0],
    tetrahedron_positions[:, 2] - tetrahedron_positions[:, 0],
    tetrahedron_positions[:, 3] - tetrahedron_positions[:, 0],
  ], axis=1)
  ...
  np.add.at(
    ...
    np.repeat(0.25 * tetrahedron_masses, 4),
  )
  ...
  return vertex_masses, tetrahedron_volumes, total_volume, density
...
(
  initial_vertex_masses,
  rest_tetrahedron_volumes,
  bunny_total_volume,
  bunny_density,
) = lump_tetrahedron_mass_to_vertices(
  bunny_positions,
  bunny_tetrahedra,
  BUNNY_TOTAL_MASS,
)
...
base_material_values = np.tile(
  np.array([YOUNG_VALUE, POISSON_VALUE], dtype=np.float64),
  (num_bunny_tetrahedra, 1),
)
...
previous_position_jacobian = add_matrices([
...
poisson_jacobian = differentiator().diff2(
  [elastic_energy],
  [position],
  [poisson],
)
young_jacobian = differentiator().diff2(
  [elastic_energy],
  [position],
  [young],
)
mass_jacobian = differentiator().diff2(
  [inertia_energy],
  [position],
  [mass],
)
...
default_output_name = (
  f"inverse_{OPTIMIZATION_TARGET.replace('-', '_')}_projected_"
...
def run_forward(
  ...
  poisson_values, poisson_chain = poisson_values_from_design(
    poisson_design
  )
  ...
  material_values = material_values_from_parameters(
    young_values,
    poisson_values,
  )
  material_summary = material_statistics(material_values)
  vertex_masses, mass_chain = mass_values_from_design(
    mass_design,
    initial_vertex_masses,
  )
  ...
  if save_this_forward:
    round_name, frame_directory, _, _ = output_paths(
      adjoint_round
    )
    design_status = (
  ...
  for frame in range(args.num_frames):
    ...
    step_displacements = np.linalg.norm(
      current_positions - saved_positions[-1],
      axis=1,
    )
...
def run_backward(trajectory, adjoint_round):
  ...
  for frame in range(args.num_frames - 1, -1, -1):
    ...
    current_state_loss_gradient_inf = float(gpuarray.max(
      abs(current_state_loss_gradient.value)
    ).get())
    current_previous_state_loss_gradient_inf = float(gpuarray.max(
      abs(current_previous_state_loss_gradient.value)
    ).get())
    state_loss_gradient_inf_min = min(
      state_loss_gradient_inf_min,
      current_state_loss_gradient_inf,
    )
    state_loss_gradient_inf_max = max(
      state_loss_gradient_inf_max,
      current_state_loss_gradient_inf,
    )
    previous_state_loss_gradient_inf_min = min(
      previous_state_loss_gradient_inf_min,
      current_previous_state_loss_gradient_inf,
    )
    previous_state_loss_gradient_inf_max = max(
      previous_state_loss_gradient_inf_max,
      current_previous_state_loss_gradient_inf,
    )
    ...
    poisson_gradient = poisson_gradient - poisson_jacobian.spmv(
      auxiliary,
      transpose=True,
    )
    young_jacobian.compute()
    young_gradient = young_gradient - young_jacobian.spmv(
      auxiliary,
      transpose=True,
    )
    mass_jacobian.compute()
    mass_gradient = mass_gradient - mass_jacobian.spmv(
      auxiliary,
      transpose=True,
    )
...
def directional_derivative_result(
  name,
  plus_loss,
  minus_loss,
  epsilon,
...
def finite_difference_check(
  ...
  young_direction = young_gradient / max(
    np.linalg.norm(young_gradient),
    1e-30,
  )
  young_epsilon = 1e-2
  young_plus_loss = run_forward(
    young_design + young_epsilon * young_direction,
    poisson_design,
    mass_design,
    adjoint_round=None,
  )["loss"]
  young_minus_loss = run_forward(
    young_design - young_epsilon * young_direction,
    poisson_design,
    mass_design,
    adjoint_round=None,
  ...
  poisson_direction = poisson_gradient / max(
    np.linalg.norm(poisson_gradient),
    1e-30,
  )
  poisson_epsilon = 1e-2
  poisson_plus_loss = run_forward(
    young_design,
    poisson_design + poisson_epsilon * poisson_direction,
    mass_design,
    adjoint_round=None,
  )["loss"]
  poisson_minus_loss = run_forward(
    young_design,
    poisson_design - poisson_epsilon * poisson_direction,
    mass_design,
    adjoint_round=None,
  ...
  mass_direction = mass_gradient / max(
    np.linalg.norm(mass_gradient),
    1e-30,
  )
  mass_epsilon = 1e-2
  mass_plus_loss = run_forward(
    young_design,
    poisson_design,
    mass_design + mass_epsilon * mass_direction,
    adjoint_round=None,
  )["loss"]
  mass_minus_loss = run_forward(
    young_design,
    poisson_design,
    mass_design - mass_epsilon * mass_direction,
    adjoint_round=None,
  ...
  return {
    "young": directional_derivative_result(
      "Young",
      young_plus_loss,
      young_minus_loss,
      young_epsilon,
    ...
    "poisson": directional_derivative_result(
      "Poisson",
      poisson_plus_loss,
      poisson_minus_loss,
      poisson_epsilon,
    ...
    "mass": directional_derivative_result(
      "Mass",
      mass_plus_loss,
      mass_minus_loss,
      mass_epsilon,
...
young_optimizer_step_size = args.young_step_size
poisson_optimizer_step_size = args.poisson_step_size
mass_optimizer_step_size = args.mass_adam_learning_rate
gradient_check = None
...
if args.resume_checkpoint is None:
  ...
  print(
    f"Resumed checkpoint round {resume_completed_adjoint_round:02d}: "
    f"stored_loss={checkpoint_current_loss:.8e} reconstructed_loss={trajectory['loss']:.8e} "
    f"relative_difference={reconstructed_relative_difference:.8e}."
  )
...
for adjoint_round in range(resume_completed_adjoint_round + 1, args.adjoint_steps + 1):
  ...
  if args.optimizer == "adam":
    young_adam_first_moment = (
      ADAM_BETA1 * young_adam_first_moment
      + (1.0 - ADAM_BETA1) * young_gradient
    )
    young_adam_second_moment = (
      ADAM_BETA2 * young_adam_second_moment
      + (1.0 - ADAM_BETA2) * young_gradient * young_gradient
    )
    poisson_adam_first_moment = (
      ADAM_BETA1 * poisson_adam_first_moment
      + (1.0 - ADAM_BETA1) * poisson_gradient
    )
    poisson_adam_second_moment = (
      ADAM_BETA2 * poisson_adam_second_moment
      + (1.0 - ADAM_BETA2) * poisson_gradient * poisson_gradient
    )
    mass_adam_first_moment = (
      ADAM_BETA1 * mass_adam_first_moment
      + (1.0 - ADAM_BETA1) * mass_gradient
    )
    mass_adam_second_moment = (
      ADAM_BETA2 * mass_adam_second_moment
      + (1.0 - ADAM_BETA2) * mass_gradient * mass_gradient
    )
    young_corrected_first = young_adam_first_moment / (
      1.0 - ADAM_BETA1 ** adjoint_round
    )
    young_corrected_second = young_adam_second_moment / (
      1.0 - ADAM_BETA2 ** adjoint_round
    )
    poisson_corrected_first = poisson_adam_first_moment / (
      1.0 - ADAM_BETA1 ** adjoint_round
    )
    poisson_corrected_second = poisson_adam_second_moment / (
      1.0 - ADAM_BETA2 ** adjoint_round
    )
    mass_corrected_first = mass_adam_first_moment / (
      1.0 - ADAM_BETA1 ** adjoint_round
    )
    mass_corrected_second = mass_adam_second_moment / (
      1.0 - ADAM_BETA2 ** adjoint_round
    )
    raw_young_update = (
      -args.adam_learning_rate
      * young_corrected_first
      / (np.sqrt(young_corrected_second) + ADAM_EPSILON)
    )
    raw_poisson_update = (
      -args.adam_learning_rate
      * poisson_corrected_first
      / (np.sqrt(poisson_corrected_second) + ADAM_EPSILON)
    )
    applied_mass_log_step = (
      -args.mass_adam_learning_rate
      * mass_corrected_first
      / (np.sqrt(mass_corrected_second) + ADAM_EPSILON)
    )
    proposed_young_step_norm = maximum_element_norm(raw_young_update)
    young_update_scale = min(
      1.0,
      args.young_step_size / proposed_young_step_norm,
    )
    applied_young_step = young_update_scale * raw_young_update
    proposed_poisson_step_norm = maximum_element_norm(
      raw_poisson_update
    )
    poisson_update_scale = min(
      1.0,
      args.poisson_step_size / proposed_poisson_step_norm,
    )
    applied_poisson_step = poisson_update_scale * raw_poisson_update
  else:
    proposed_young_step_norm = young_optimizer_step_size
    applied_young_step = (
      -young_optimizer_step_size
      * young_gradient
      / maximum_element_norm(young_gradient)
    )
    proposed_poisson_step_norm = poisson_optimizer_step_size
    applied_poisson_step = (
      -poisson_optimizer_step_size
      * poisson_gradient
      / maximum_element_norm(poisson_gradient)
    )
    applied_mass_log_step = (
      -mass_optimizer_step_size
      * mass_gradient
      / np.max(np.abs(mass_gradient))
    )

  young_design = young_design + applied_young_step
  young_design = np.clip(
    young_design,
    -YOUNG_LOGIT_DESIGN_LIMIT,
    YOUNG_LOGIT_DESIGN_LIMIT,
  )
  poisson_design = poisson_design + applied_poisson_step
  poisson_design = np.clip(
    poisson_design,
    -POISSON_LOGIT_DESIGN_LIMIT,
    POISSON_LOGIT_DESIGN_LIMIT,
  )
  ...
  if args.optimizer == "history-gradient":
    if loss_decreased:
      young_optimizer_step_size = min(
        args.young_step_size,
        STEP_SIZE_GROWTH * young_optimizer_step_size,
      )
      poisson_optimizer_step_size = min(
        args.poisson_step_size,
        STEP_SIZE_GROWTH * poisson_optimizer_step_size,
      )
      mass_optimizer_step_size = min(
        args.mass_adam_learning_rate,
        STEP_SIZE_GROWTH * mass_optimizer_step_size,
      )
    else:
      young_optimizer_step_size *= STEP_SIZE_SHRINK
      poisson_optimizer_step_size *= STEP_SIZE_SHRINK
      mass_optimizer_step_size *= STEP_SIZE_SHRINK
  ...
  record = {
    ...
    "maximum_poisson_step_norm": maximum_element_norm(
      applied_poisson_step
    ),
    "mass_log_step_inf": float(np.max(np.abs(applied_mass_log_step))),
    "next_young_step_size": young_optimizer_step_size,
    "next_poisson_step_size": poisson_optimizer_step_size,
    "next_mass_log_step_size": mass_optimizer_step_size,
...
best_material_values = material_values_from_parameters(
  best_young_values,
  best_poisson_values,
)
