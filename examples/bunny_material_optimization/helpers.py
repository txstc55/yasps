...
# Extract the boundary topology used by collision detection and rendering.
def extract_surface_triangles(tets):
  ...
  return np.array([list(face) for face, count in face_count.items() if count == 1], dtype=np.uint32)
...
# Build the per-tetrahedron elastic energy used by the soft bunny.
...
# Predict the next position under inertia and gravity.
def inertia(last_position, velocity, dt, position, mass):
  target = last_position + velocity * dt - attribute.to_array([0.0, 9.8 * dt * dt, 0.0], rows=3, cols=1)
  difference = position - target
  return 0.5 * difference.transpose() * mass * difference


# IPC barrier energies for each primitive-pair type.
