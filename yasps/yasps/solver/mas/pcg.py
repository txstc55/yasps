...
def pcg(
  ...
  absolute_target = tolerance * denominator
  residual_norm = float(np.linalg.norm(residual))
  if residual_norm <= absolute_target:
    return PCGResult(x, 0, residual_norm, residual_norm / denominator, True, perf_counter() - started)
  if max_iterations == 0:
    return PCGResult(x, 0, residual_norm, residual_norm / denominator, False, perf_counter() - started)
