...
{kernelRawName}{{
  ...
  for (unsigned int i = 0; i < {len(self.__block_sizes)}; i++){{
    ...
    for (unsigned int j = i; j < {len(self.__block_sizes)}; j++){{
      ...
      unsigned int where_to_check = raw_position_i <= raw_position_j ? hessian_blocks_where_to_check[off_diagonal_counts] : hessian_blocks_where_to_check[off_diagonal_counts + {((len(self.__block_sizes) + 1) * len(self.__block_sizes)) // 2}]; // know which off diagonal block we are in
