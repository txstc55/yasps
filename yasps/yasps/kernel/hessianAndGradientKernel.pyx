...
class hessianAndGradientKernel:
  ...
  // determine if we are computing both the hessian and gradient
    ...
    for (unsigned int i = 0; i < max_num_indices; i++){{
      unsigned int col_offset = 0;
      // we first determine what's the correct position to put in the compressed hessian
      short int permutation_i = permutation[instance * max_num_indices + i]; // get the permuted placement
      if (permutation_i > 0){{
        ...
        for (unsigned int j = i; j < max_num_indices; j++){{
          short int permutation_j = permutation[instance * max_num_indices + j]; // get the permuted placement
          if (permutation_j > 0){{
            ...
            for (unsigned int k = 0; k < segment_size_i; k++){{
              for (unsigned int l = 0; l < segment_size_j; l++){{
                if (segment_index_i < segment_index_j){{
                  ...
                  atomicAdd(&hessian_blocks[placement_index + k * segment_size_j + l], hg_mat(permutation_j + l, permutation_i + k));
                }}
              }}
            }}
            ...
            if (i == j){{
              ...
              for (unsigned int k = 0; k < segment_size_i; k++){{
                atomicAdd(&diagonal[segment_index + k], hg_mat(permutation_i + k, permutation_j + k));
              }}
            }}
          }}
        }}
  ...
  '''
      ...
      for size in self.__unique_gradient_sizes:
        self.__kernelString += f'''
        case {size}:
          compute_hessian_and_gradient_global_function<{size}><<<(unique_gradient_sizes_instance_count[i] + 255) / 256, 256, streams[i]>>>(
            {"".join([f"{x.code_generation_data_name}, " for x in sortedDatas])}
            {"".join([f"{x.code_generation_index_name}, " for x in sortedConnectivities])}
            {"".join([f"{x.code_generation_csr_name}, " for x in sortedConnectivities if x.dimension == 0])}
  ...
  def compute(
    ...
    self.__kernel(
      *[self.__to_void_p(x) for x in attributeArgs],
      ...
      ctypes.c_uint32(0),
      self.__to_void_p(giKernel.maxNumIndicesNeeded),
      ...
      ctypes.c_uint32(giKernel.outputNumUniqueGradientSizesCPU)
    )
